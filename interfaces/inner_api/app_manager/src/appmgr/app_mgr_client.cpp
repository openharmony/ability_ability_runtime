...
AppMgrResultCode AppMgrClient::KillApplicationWithUserId(const std::string &bundleName,
    const int userId, const int appIndex)
{
    sptr<IAppMgr> service = iface_cast<IAppMgr>(mgrHolder_->GetRemoteObject());
    if (service != nullptr) {
        sptr<IAmsMgr> amsService = service->GetAmsMgr();
        if (amsService != nullptr) {
            int32_t result = amsService->KillApplicationWithUserId(bundleName, userId, appIndex);
            if (result == ERR_OK) {
                return AppMgrResultCode::RESULT_OK;
            }
            return AppMgrResultCode::ERROR_SERVICE_NOT_READY;
...
bool AppMgrClient::IsMainProcessDebug(int32_t uid)
{
    sptr<IAppMgr> service = iface_cast<IAppMgr>(mgrHolder_->GetRemoteObject());
    if (service == nullptr) {
        return false;
...
int32_t AppMgrClient::EnableDelayedProcessExit(bool enabled) const
{
    sptr<IAppMgr> service = iface_cast<IAppMgr>(mgrHolder_->GetRemoteObject());
    if (service == nullptr) {
        TAG_LOGE(AAFwkTag::APPMGR, "Service is nullptr.");
    ...
    return service->EnableDelayedProcessExit(enabled);
}
...
int32_t AppMgrClient::KillChildProcessByPid(int32_t pid)
{
    if (mgrHolder_ == nullptr) {
        TAG_LOGE(AAFwkTag::APPMGR, "mgrHolder is nullptr.");
        return ERROR_SERVICE_NOT_CONNECTED;
