...
#include "appforegroundstateobserverproxy_fuzzer.h"
...
bool DoSomethingInterestingWithMyAPI(const char* data, size_t size)
{
    ...
    auto appForegroundStateObserverProxy = std::make_shared<AppForegroundStateObserverProxy>(impl);
    AppStateData appStateData;
    appForegroundStateObserverProxy->OnAppStateChanged(appStateData);
    MessageParcel dataParcel;
    dataParcel.WriteInterfaceToken(APPMGR_INTERFACE_TOKEN);
    dataParcel.WriteBuffer(data, size);
    dataParcel.RewindRead(0);
    appForegroundStateObserverProxy->WriteInterfaceToken(dataParcel);
