...
#include "scheduledisconnectabilitydone_fuzzer.h"
...
bool DoSomethingInterestingWithMyAPI(const char* data, size_t size)
{
    ...
    if (connectManager) {
        connectManager->ScheduleDisconnectAbilityDoneLocked(serviceToken);
    }

    if (abilitymgr->ScheduleDisconnectAbilityDone(token) != 0) {
        return false;
    }
