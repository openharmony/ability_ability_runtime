...
using StartupTaskMap = std::map<std::string, std::shared_ptr<StartupTask>>;
class StartupManager : public std::enable_shared_from_this<StartupManager> {
    DECLARE_DELAYED_SINGLETON(StartupManager)
public:
    MOCK_METHOD(int32_t, BuildStartupTaskManager, (const StartupTaskMap&, std::shared_ptr<StartupTaskManager>&));
};

inline StartupManager::StartupManager() = default;
inline StartupManager::~StartupManager() = default;
} // namespace OHOS::AbilityRuntime
