...
public:
    ...
    MOCK_METHOD4(ContinueAbility, int(const std::string& deviceId, int32_t missionId, uint32_t versionCode,
        int32_t userId));
    ...
    virtual int StartUser(int userId, uint64_t displayId, sptr<IUserCallback> callback, bool isAppRecovery) override
    {
        return 0;
    }
