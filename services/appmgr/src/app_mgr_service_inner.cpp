...
void AppMgrServiceInner::ApplicationForegrounded(const int32_t recordId)
{
    ...
    TAG_LOGI(AAFwkTag::APPMGR, "%{public}s is foregrounded", appRecord->GetBundleName().c_str());
...
int32_t AppMgrServiceInner::SetSupportedProcessCacheSelf(int32_t pid, bool isSupport)
{
    TAG_LOGI(AAFwkTag::APPMGR, "called");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);
    if (!appRunningManager_) {
