    ...
    std::string bundleName, std::function<void()> callback, bool isDialogExist)
{
    ...
    TAG_LOGI(AAFwkTag::ABILITYMGR,
        "%{public}s is %{public}s.pid:%{public}s lastFreezePid:%{public}s,timeout %{public}lu",
        bundleName.c_str(), focusFlag ? "focus" : "not focus", pid.c_str(), lastFreezePid.c_str(), now - lastFreezeTime);
    bool isPullUpBox =
