...
int AbilityManagerProxy::SetComponentInterception(const sptr<AppExecFwk::IComponentInterception> &componentInterception)
{
    ...
    auto error = SendRequest(AbilityManagerInterfaceCode::SET_COMPONENT_INTERCEPTION, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("Send request error: %{public}d", error);
        return error;
    }
    ...
    int32_t requestCode, int32_t resultCode, int32_t userId)
{
    ...
    auto error = SendRequest(AbilityManagerInterfaceCode::SEND_ABILITY_RESULT_BY_TOKEN, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("Send request error: %{public}d", error);
        return error;
    }
...
bool AbilityManagerProxy::NotifyConfigurationChange(const AppExecFwk::Configuration &config, int32_t userId)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    auto error = SendRequest(AbilityManagerInterfaceCode::NOTIFY_CONFIGURATION_CHANGE, data, reply, option);
    if (error != NO_ERROR) {
...
int AbilityManagerProxy::OpenFile(const Uri& uri, uint32_t flag)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
