...
void AbilityManagerStub::SanitizeWantParams(Want &want)
{
...
int32_t AbilityManagerStub::GetInsightIntentInfoByIntentNameInner(MessageParcel &data, MessageParcel &reply)
{
    ...
    int32_t result = GetInsightIntentInfoByIntentName(flag, bundleName, modlueName, intentName, info, userId);
    if (!reply.WriteParcelable(&info)) {
        return INNER_ERR;
    }
    if (!reply.WriteInt32(result)) {
...
int AbilityManagerStub::StartSelfUIAbilityInChildProcessInner(MessageParcel &data, MessageParcel &reply)
{
    std::shared_ptr<Want> want(data.ReadParcelable<Want>());
    if (want == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITYMGR, "want null");
    ...
    int32_t result = StartSelfUIAbilityInChildProcess(*want, specifiedFlag, callerToken);
    reply.WriteInt32(result);
    return NO_ERROR;
}
