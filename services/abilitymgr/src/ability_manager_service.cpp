...
int AbilityManagerService::CloseUIAbilityBySCB(const sptr<SessionInfo> &sessionInfo, uint32_t sceneFlag)
{
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    if (sessionInfo == nullptr || sessionInfo->sessionToken == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITYMGR, "sessionInfo null");
    ...
    const sptr<IAbilityScheduler> &scheduler, const sptr<IRemoteObject> &token)
{
    ...
    if (type == AppExecFwk::AbilityType::SERVICE || type == AppExecFwk::AbilityType::EXTENSION) {
        ...
        if (!dataAbilityManager) {
            TAG_LOGE(AAFwkTag::SERVICE_EXT, "dataAbilityManager null userId=%{public}d", userId);
            return ERR_INVALID_VALUE;
        }
...
int AbilityManagerService::AbilityTransitionDone(const sptr<IRemoteObject> &token, int state, const PacMap &saveData)
{
    ...
    TAG_LOGI(AAFwkTag::SERVICE_EXT, "AbilityTransitionDone, ability: %{public}s, state:%{public}d",
    ...
    if (IsNeedTimeoutForTest(abilityInfo.name, tempState)) {
        TAG_LOGW(AAFwkTag::SERVICE_EXT, "force timeout ability, state:%{public}s, ability:%{public}s",
            tempState.c_str(),
...
int32_t AbilityManagerService::CleanUIAbilityBySCB(const sptr<SessionInfo> &sessionInfo, uint32_t sceneFlag)
{
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    if (sessionInfo == nullptr || sessionInfo->sessionToken == nullptr) {
