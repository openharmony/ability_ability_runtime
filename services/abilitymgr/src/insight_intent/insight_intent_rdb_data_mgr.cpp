...
bool InsightIntentRdbDataMgr::InitIntentTable(const IntentRdbConfig &intentRdbConfig)
{
    ...
    if (ret != NativeRdb::E_OK) {
        TAG_LOGE(AAFwkTag::INTENT, "Create rdb table failed, ret:%{public}" PRId32 "", ret);
        return false;
    }
...
std::shared_ptr<NativeRdb::RdbStore> InsightIntentRdbDataMgr::GetRdbStore()
{
    ...
    std::string rdbPath = intentRdbConfig_.dbPath + intentRdbConfig_.dbName;
    ...
    if (rebuildType == NativeRdb::RebuiltType::REBUILT || isNeedRebuildDb) {
        TAG_LOGI(AAFwkTag::INTENT, "start %{public}s restore ret %{public}d, type:%{public}d", intentRdbConfig_.dbName.c_str(),
...
void InsightIntentRdbDataMgr::BackupRdb()
{
    ...
    auto ret = rdbStore->Backup(intentRdbConfig_.dbPath + std::string("/") + std::string(INTENT_BACK_UP_RDB_NAME));
    if (ret != NativeRdb::E_OK) {
        TAG_LOGE(AAFwkTag::INTENT, "Backup failed, errCode:%{public}d", ret);
    }
    TAG_LOGI(AAFwkTag::INTENT, "%{public}s backup end", intentRdbConfig_.dbName.c_str());
}
...
{
    TAG_LOGI(AAFwkTag::INTENT, "OnCreate");
...
{
    TAG_LOGI(AAFwkTag::INTENT, "OnUpgrade currentVersion: %{public}d, targetVersion: %{public}d",
...
int32_t IntentRdbOpenCallback::OnDowngrade(
    NativeRdb::RdbStore &rdbStore, int currentVersion, int targetVersion)
{
    TAG_LOGI(AAFwkTag::INTENT, "OnDowngrade  currentVersion: %{public}d, targetVersion: %{public}d",
        currentVersion, targetVersion);
    return NativeRdb::E_OK;
}
...
{
    TAG_LOGI(AAFwkTag::INTENT, "OnOpen");
    return NativeRdb::E_OK;
}

int32_t IntentRdbOpenCallback::onCorruption(std::string databaseFile)
{
    TAG_LOGI(AAFwkTag::INTENT, "onCorruption");
    return NativeRdb::E_OK;
}
