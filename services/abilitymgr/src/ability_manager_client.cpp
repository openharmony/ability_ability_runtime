...
#define CHECK_POINTER_RETURN(object)                        \
    if (!object) {                                          \
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "proxy is nullptr"); \
        return;                                             \
    }
    ...
    const Want &want, sptr<IRemoteObject> callerToken, int requestCode, int32_t userId)
{
    ...
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
    TAG_LOGD(AAFwkTag::SERVICE_EXT, "ability:%{public}s, userId:%{public}d",
    ...
    const Want &want, sptr<IRemoteObject> callerToken, int requestCode, int32_t userId)
{
    ...
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
    TAG_LOGD(AAFwkTag::SERVICE_EXT, "The abilityName:%{public}s, userId:%{public}d",
        want.GetElement().GetAbilityName().c_str(), userId);
    HandleDlpApp(const_cast<Want &>(want));
    ...
    sptr<IAbilityConnection> connect, int32_t userId)
{
    ...
    auto abms = GetAbilityManager();
    if (abms == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Connect failed, bundleName:%{public}s, abilityName:%{public}s, uri:%{public}s.",
    ...
    int32_t userId)
{
    ...
    auto abms = GetAbilityManager();
    if (abms == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Connect failed, bundleName:%{public}s, abilityName:%{public}s",
            want.GetElement().GetBundleName().c_str(), want.GetElement().GetAbilityName().c_str());
        return ABILITY_SERVICE_NOT_CONNECTED;
    }
...
ErrCode AbilityManagerClient::Connect()
{
    ...
    if (systemManager == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Fail to get registry.");
        return GET_ABILITY_SERVICE_FAILED;
    }
    sptr<IRemoteObject> remoteObj = systemManager->GetSystemAbility(ABILITY_MGR_SERVICE_ID);
    if (remoteObj == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Fail to connect ability manager service.");
        return GET_ABILITY_SERVICE_FAILED;
    }

    deathRecipient_ = sptr<IRemoteObject::DeathRecipient>(new AbilityMgrDeathRecipient());
    if (deathRecipient_ == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Failed to create AbilityMgrDeathRecipient!");
        return GET_ABILITY_SERVICE_FAILED;
    }
    if ((remoteObj->IsProxyObject()) && (!remoteObj->AddDeathRecipient(deathRecipient_))) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "Add death recipient to AbilityManagerService failed.");
        return GET_ABILITY_SERVICE_FAILED;
    }
...
void AbilityManagerClient::RemoveDeathRecipient()
{
    ...
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    if (proxy_ == nullptr) {
        TAG_LOGI(AAFwkTag::SERVICE_EXT, "AbilityMgrProxy do not exist");
        return;
    }
    if (deathRecipient_ == nullptr) {
        TAG_LOGI(AAFwkTag::SERVICE_EXT, "AbilityMgrDeathRecipient do not exist");
        return;
    }
    ...
    if (serviceRemote != nullptr && serviceRemote->RemoveDeathRecipient(deathRecipient_)) {
        ...
        TAG_LOGI(AAFwkTag::SERVICE_EXT, "Remove DeathRecipient success");
    }
}
    ...
    const Want &want, sptr<IRemoteObject> callerToken)
{
    ...
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
    TAG_LOGI(AAFwkTag::SERVICE_EXT, "request is:%{public}s.", want.GetElement().GetURI().c_str());
    HandleDlpApp(const_cast<Want &>(want));
...
{
    TAG_LOGI(AAFwkTag::SERVICE_EXT, "StartUser in client:%{public}d.", accountId);
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
...
{
    TAG_LOGI(AAFwkTag::SERVICE_EXT, "StopUser in client:%{public}d.", accountId);
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
    ...
    bool isNeedLocalDeviceId)
{
    ...
    auto abms = GetAbilityManager();
    if (abms == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "abms == nullptr");
        return {};
    }
    ...
    sptr<IPrepareTerminateCallback> callback)
{
    ...
    auto abms = GetAbilityManager();
    if (abms == nullptr) {
        TAG_LOGE(AAFwkTag::SERVICE_EXT, "abms is nullptr.");
        return ERR_INVALID_VALUE;
    }
    ...
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    {
        ...
        if (Rosen::SceneBoardJudgement::IsSceneBoardEnabled()) {
            ...
            if (sceneSessionManager == nullptr) {
                TAG_LOGE(AAFwkTag::SERVICE_EXT, "Failed to get sceneSessionManager.");
                return elementName;
            }
            TAG_LOGI(AAFwkTag::SERVICE_EXT, "scb call, GetTopAbility element");
...
{
    TAG_LOGI(AAFwkTag::SERVICE_EXT, "begin.");
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
