...
- **DESIGN — ScreenUnlock consumes `param.abilityInfo` directly:** the interceptor no longer resolves its target itself — `GetTargetAbilityInfo` / `QueryTargetAbilityInfo` / `QueryTargetAbilityInfoByUri` (cache / BMS / URI fallbacks, pre-merge pre-check relics) and the `ScreenUnlockCtx` fromConnect marker were deleted. Every merged-chain site passes the resolved target via `.AbilityInfo(...)`; the two remote-dispatch sites seed it from the wrap cache (see the remote-dispatch DESIGN entry). A null `abilityInfo` is a no-op. The connect path needs no URI lookup: `ConnectLocalAbility` resolves the request (URI wants included) before the chain runs, and the pre-merge URI path was only reachable from the pre-resolution pre-check, which no longer exists.

- **BENIGN — ScreenUnlock re-add drift (P2-1, not-a-finding):** `InitInterceptorForScreenUnlock` re-adds ScreenUnlock on a `StartUser` lock event; if it was previously removed (screen-unlock callback at `ams.cpp:10077/10114`), the re-add appends it to the chain end. This drift is benign: ScreenUnlock's block/pass decision is position-independent (`param.abilityInfo`, `IsScreenLocked()`, `isSystemApp`/extension do not depend on chain order), and no upstream interceptor mutates `param.want` while returning `ERR_OK` (EcologicalRule mutates then returns `ERR_ECOLOGICAL_CONTROL_STATUS`, breaking the chain). So no target that ScreenUnlock-at-front would block is let through at the end. Do not add a front/position mechanism for ScreenUnlock — it is unnecessary and conflicts with the push_back-only vector design.
