...
NativeValue* StartTimeoutOrInterval(NativeEngine* engine, NativeCallbackInfo* info, bool isInterval)
{
    if (engine == nullptr || info == nullptr) {
        HILOG_ERROR("StartTimeoutOrInterval, Start timeout or interval failed with engine or callback info is nullptr.");
        return nullptr;
    }
...
{
    if (engine == nullptr || info == nullptr) {
        HILOG_ERROR("StopTimeoutOrInterval, Stop timeout or interval failed with engine or callback info is nullptr.");
        return nullptr;
    }
