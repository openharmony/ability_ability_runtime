...
bool IsTypeForNapiValue(napi_env env, napi_value param, napi_valuetype expectType)
{
    ...
    if (param == nullptr) {
        TAG_LOGD(AAFwkTag::BRIDGE, "param is nullptr");
        return false;
    }
...
bool IsArrayForNapiValue(napi_env env, napi_value param, uint32_t &arraySize)
{
    ...
    if (napi_get_array_length(env, param, &arraySize) != napi_ok) {
        TAG_LOGD(AAFwkTag::BRIDGE, "napi_get_array_length is 0");
        return false;
    }
    return true;
}
...
{
    TAG_LOGD(AAFwkTag::BRIDGE, "UnwrapStringFromJS enter");
    size_t size = 0;
    ...
    std::string value("");
    if (size == 0) {
        TAG_LOGD(AAFwkTag::BRIDGE, "size is 0");
        return defaultValue;
    }

    char *buf = new (std::nothrow) char[size + 1];
    if (buf == nullptr) {
        TAG_LOGD(AAFwkTag::BRIDGE, "buf is nullptr");
        return value;
    }
...
bool UnwrapArrayInt32FromJS(napi_env env, napi_value param, std::vector<int> &value)
{
    ...
    if (!IsArrayForNapiValue(env, param, arraySize)) {
        TAG_LOGD(AAFwkTag::BRIDGE, "arraySize Expanding ArrayInt32 from JS failed");
        return false;
    }
    ...
    for (uint32_t i = 0; i < arraySize; i++) {
        ...
        if (napi_get_element(env, param, i, &jsValue) != napi_ok) {
            TAG_LOGD(AAFwkTag::BRIDGE, "array item Expanding ArrayInt32 from JS failed");
            return false;
        }
...
bool UnwrapArrayInt64FromJS(napi_env env, napi_value param, std::vector<int64_t> &value)
{
    ...
    if (!IsArrayForNapiValue(env, param, arraySize)) {
        TAG_LOGD(AAFwkTag::BRIDGE, "arraySize Expanding ArrayInt32 from JS error");
        return false;
    }
    ...
    for (uint32_t i = 0; i < arraySize; i++) {
        ...
        natValue = 0;
        if (napi_get_element(env, param, i, &jsValue) != napi_ok) {
            TAG_LOGD(AAFwkTag::BRIDGE, "array item Expanding ArrayInt32 from JS error");
            return false;
        }
    ...
    napi_env env, napi_value jsObject, const char *propertyName, std::vector<int32_t> &value)
{
    ...
    return UnwrapArrayInt32FromJS(env, jsArray, value);
}
...
napi_ref CreateCallbackRefFromJS(napi_env env, napi_value param)
{
    if (env == nullptr || param == nullptr) {
        TAG_LOGI(AAFwkTag::BRIDGE, "null env or param");
        return nullptr;
    }
    ...
    if (valueType != napi_function) {
        TAG_LOGI(AAFwkTag::BRIDGE, "invalid param");
        return nullptr;
    }
...
napi_value ExecuteAsyncCallbackWork(napi_env env, AsyncJSCallbackInfo *asyncCallbackInfo, const AsyncParamEx *param)
{
    ...
    if (asyncCallbackInfo == nullptr || param == nullptr) {
        TAG_LOGI(AAFwkTag::BRIDGE, "null asyncCallbackInfo or param");
        return nullptr;
    }
...
std::vector<std::string> ConvertStringVector(napi_env env, napi_value jsValue)
{
    ...
    if (status != napi_ok || !isTypedArray) {
        TAG_LOGE(AAFwkTag::BRIDGE, "isTypedArray error");
        return {};
    }
    ...
    NAPI_CALL_BASE(env, napi_get_typedarray_info(env, jsValue, &type, &length, nullptr, &buffer, &offset), {});
    if (type != napi_uint8_array) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null napi_uint8_array");
        return {};
    }
...
napi_value WrapLocaleToJS(napi_env env, const std::string &locale)
{
    ...
    if (status != napi_ok || global == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Load global failed");
        return nullptr;
    }
    ...
    if (status != napi_ok || intl == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Load Intl failed");
        return nullptr;
    }
    ...
    if (status != napi_ok || localeConstructor == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Load Intl.Locale constructor failed");
        return nullptr;
    }

    napi_value localeJS = WrapStringToJS(env, locale);
    if (localeJS == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Wrap locale failed");
        return nullptr;
    }
    ...
    if (status != napi_ok || intlLocale == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Create Intl.Locale instance failed");
        return nullptr;
    }
    return intlLocale;
}
...
bool UnwrapLocaleFromJS(napi_env env, napi_value jsValue, std::string &value)
{
    ...
    if (status != napi_ok || func == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Get function toString failed");
        return false;
    }
    ...
    if (status != napi_ok || locale == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Call function toString failed");
        return false;
    }
