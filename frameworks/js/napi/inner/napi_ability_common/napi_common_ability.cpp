...
napi_value NAPI_GetFilesDirWrap(napi_env env, napi_callback_info info, AsyncJSCallbackInfo *asyncCallbackInfo)
{
    ...
    if (argc == ARGS_ONE) {
        if (!CreateAsyncCallback(env, args[PARAM0], asyncCallbackInfo)) {
            TAG_LOGE(AAFwkTag::BRIDGE, "invalid args[PARAM0]");
            return nullptr;
        }
    }
...
napi_value NAPI_GetOrCreateDistributedDirCommon(napi_env env, napi_callback_info info, AbilityType abilityType)
{
    ...
    AsyncJSCallbackInfo *asyncCallbackInfo = CreateAsyncJSCallbackInfo(env);
    if (asyncCallbackInfo == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null asyncCallback");
        return WrapVoidToJS(env);
    }
...
napi_value NAPI_GetExternalCacheDirWrap(napi_env env, napi_callback_info info, AsyncJSCallbackInfo *asyncCallbackInfo)
{
    ...
    if (argc == ARGS_ONE) {
        if (!CreateAsyncCallback(env, args[PARAM0], asyncCallbackInfo)) {
            TAG_LOGE(AAFwkTag::BRIDGE, "args[PARAM0] invalid");
            return nullptr;
        }
    }
...
napi_value NAPI_GetExternalCacheDirCommon(napi_env env, napi_callback_info info, AbilityType abilityType)
{
    ...
    AsyncJSCallbackInfo *asyncCallbackInfo = CreateAsyncJSCallbackInfo(env);
    if (asyncCallbackInfo == nullptr) {
        TAG_LOGI(AAFwkTag::BRIDGE, "create callbackInfo failed");
        return WrapVoidToJS(env);
    }
    ...
    napi_env env, napi_callback_info info, AsyncJSCallbackInfo *asyncCallbackInfo)
{
    ...
    if (argc == ARGS_ONE) {
        if (!CreateAsyncCallback(env, args[PARAM0], asyncCallbackInfo)) {
            TAG_LOGI(AAFwkTag::BRIDGE, "args[PARAM0] invalid");
            return nullptr;
        }
    }
...
napi_value NAPI_IsUpdatingConfigurationsCommon(napi_env env, napi_callback_info info, AbilityType abilityType)
{
    ...
    AsyncJSCallbackInfo *asyncCallbackInfo = CreateAsyncJSCallbackInfo(env);
    if (asyncCallbackInfo == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "create callbackInfo failed");
        return WrapVoidToJS(env);
    }
...
napi_value GetDisplayOrientationWrap(napi_env env, napi_callback_info info, AsyncJSCallbackInfo *asyncCallbackInfo)
{
    ...
    if (!UnwrapParamGetDisplayOrientationWrap(env, argc, args, asyncCallbackInfo)) {
        TAG_LOGI(AAFwkTag::BRIDGE, "unwrapParamGetDisplayOrientationWrap fail");
        return nullptr;
    }
...
void GetDisplayOrientationExecuteCallback(napi_env env, void *data)
{
    ...
    if (!CheckAbilityType(asyncCallbackInfo)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "invalid type");
...
AsyncCallbackInfo *CreateAsyncCallbackInfo(napi_env env)
{
    TAG_LOGI(AAFwkTag::BRIDGE, "called");
    if (env == nullptr) {
        TAG_LOGI(AAFwkTag::BRIDGE, "null env");
        return nullptr;
    }
    ...
    if (ret != napi_ok) {
        ...
        TAG_LOGE(AAFwkTag::BRIDGE, "get_global=%{public}d err:%{public}s", ret,
    ...
    if (ret != napi_ok) {
        napi_get_last_error_info(env, &errorInfo);
        TAG_LOGE(AAFwkTag::BRIDGE, "get_named_property=%{public}d err:%{public}s", ret,
                 errorInfo->error_message);
    }
    ...
    if (ret != napi_ok) {
        napi_get_last_error_info(env, &errorInfo);
        TAG_LOGE(AAFwkTag::BRIDGE, "get_value_external=%{public}d err:%{public}s", ret,
                 errorInfo->error_message);
    }
    ...
    napi_env env, napi_value *args, const size_t argCallback, AsyncCallbackInfo *asyncCallbackInfo)
{
    ...
    if (args == nullptr || asyncCallbackInfo == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null params");
        return nullptr;
    }
    ...
    if (valuetype == napi_function) {
        TAG_LOGD(AAFwkTag::BRIDGE, "napi_create_reference");
    ...
    napi_create_async_work(env, nullptr, resourceName, GetContextAsyncExecuteCB,
        [](napi_env env, napi_status, void *data) {
            ...
            if (asyncCallbackInfo->cbInfo.callback != nullptr) {
                TAG_LOGD(AAFwkTag::BRIDGE, "Delete GetContextAsync callback reference");
    ...
    TAG_LOGI(AAFwkTag::BRIDGE, "asyncCallback end");
    return result;
}
...
{
    if (dataAbilityHelperCB == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null dataAbilityHelperCB");
        return nullptr;
    }
    ...
    if (status == napi_ok) {
        uriIndex = PARAM1;
        TAG_LOGI(AAFwkTag::BRIDGE, "Stage Model: %{public}d", stageMode);
    }

    if (!stageMode) {
        auto ability = OHOS::AbilityRuntime::GetCurrentAbility(env);
        if (ability == nullptr) {
            TAG_LOGE(AAFwkTag::BRIDGE, "Failed to get native context instance");
            return nullptr;
        }
        ...
        if (!CheckAbilityType(&dataAbilityHelperCB->cbBase)) {
            ...
            TAG_LOGE(AAFwkTag::BRIDGE, "ability type invalid");
            return nullptr;
        }
    }
    napi_valuetype valuetype = napi_undefined;
    ...
    if (valuetype != napi_string) {
        TAG_LOGE(AAFwkTag::BRIDGE, "argument type error");
        return nullptr;
    }
    napi_value result = nullptr;
    ...
    if (!IsTypeForNapiValue(env, result, napi_object)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "not object");
        return nullptr;
    }

    if (IsTypeForNapiValue(env, result, napi_null)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "result null");
        return nullptr;
    }

    if (IsTypeForNapiValue(env, result, napi_undefined)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "result undefined");
        return nullptr;
    }

    if (!GetDataAbilityHelperStatus()) {
        TAG_LOGE(AAFwkTag::BRIDGE, "GetDataAbilityHelperStatus false");
        return nullptr;
    }
...
napi_value NAPI_StartBackgroundRunningCommon(napi_env env, napi_callback_info info)
{
    ...
    TAG_LOGI(AAFwkTag::BRIDGE, "finish");
    return ret;
}
