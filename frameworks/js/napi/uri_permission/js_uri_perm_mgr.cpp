...
private:
    napi_value OnGrantUriPermission(napi_env env, NapiCallbackInfo& info)
    {
        ...
        if (!OHOS::AppExecFwk::UnwrapStringFromJS2(env, info.argv[0], uriStr)) {
            ...
            ThrowError(env, AbilityErrorCode::ERROR_CODE_INVALID_PARAM,  "Parameter error. The type of uri must be string.");
            return CreateJsUndefined(env);
        }
        int flag = 0;
        if (!OHOS::AppExecFwk::UnwrapInt32FromJS2(env, info.argv[1], flag)) {
            TAG_LOGE(AAFwkTag::URIPERMMGR, "The flag is invalid.");
            ThrowError(env, AbilityErrorCode::ERROR_CODE_INVALID_PARAM,  "Parameter error. The type of flag must be number.");
            return CreateJsUndefined(env);
        }
        ...
        if (!OHOS::AppExecFwk::UnwrapStringFromJS2(env, info.argv[argCountTwo], targetBundleName)) {
            ...
            ThrowError(env, AbilityErrorCode::ERROR_CODE_INVALID_PARAM,  "Parameter error. The type of targetBundleName must be string.");
            return CreateJsUndefined(env);
        }
    ...
    napi_value OnRevokeUriPermission(napi_env env, NapiCallbackInfo& info)
    {
        ...
        if (!OHOS::AppExecFwk::UnwrapStringFromJS2(env, info.argv[1], bundleName)) {
            ...
            ThrowError(env, AbilityErrorCode::ERROR_CODE_INVALID_PARAM, "Parameter error. The type of bundleName must be string.");
            return CreateJsUndefined(env);
        }
        auto selfToken = IPCSkeleton::GetSelfTokenID();
        if (!Security::AccessToken::TokenIdKit::IsSystemAppByFullTokenID(selfToken)) {
