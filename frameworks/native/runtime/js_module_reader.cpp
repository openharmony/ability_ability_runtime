...
{
    HILOG_INFO("JsModuleReader operator start: %{public}s", inputPath.c_str());
    ...
    if (extractor == nullptr) {
        HILOG_ERROR("realHapPath %{public}s GetExtractor failed", realHapPath.c_str());
        return buffer;
    }
...
std::string JsModuleReader::GetFormAppHspPath(const std::string& inputPath) const
{
    ...
    HILOG_INFO("realHapPath: %{public}s", realHapPath.c_str());
    if (realHapPath.empty() ||
...
std::string JsModuleReader::GetOtHsp(const std::string& inputPath) const
{
    return inputPath.substr(inputPath.find("bundle/") + 7);
}
