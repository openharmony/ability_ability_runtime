...
#endif // SUPPORT_SCREEN
void UIExtensionCallback::OnRequestSuccess(const std::string& name)
{
    TAG_LOGI(AAFwkTag::UI_EXT, "call OnRequestSuccess, name:%{public}s", name.c_str());
}

void UIExtensionCallback::OnRequestFailure(
    const std::string &name, int32_t failureCode, const std::string &failureMessage)
{
    TAG_LOGI(AAFwkTag::UI_EXT,
        "call OnRequestFailure, name:%{public}s failureCode:%{public}d failureMessage:%{public}s", name.c_str(),
        failureCode, failureMessage.c_str());
}
} // namespace AbilityRuntime
} // namespace OHOS
