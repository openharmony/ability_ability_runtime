...
AgentConnectorProxy::AgentConnectorProxy(const sptr<IRemoteObject> &impl)
    : IRemoteProxy<IAgentConnector>(impl)
{
}
...
int32_t AgentConnectorProxy::SendData(const std::string &data)
{
    ...
    if (!parcelData.WriteInterfaceToken(AgentConnectorProxy::GetDescriptor())) {
    ...
    auto error = remoteObject->SendRequest(static_cast<uint32_t>(IAgentConnector::SEND_DATA),
...
int32_t AgentConnectorProxy::Authorize(const std::string &data)
{
    ...
    auto error = remoteObject->SendRequest(static_cast<uint32_t>(IAgentConnector::AUTHORIZE),
        parcelData, reply, option);
    if (error != ERR_OK) {
        TAG_LOGE(AAFwkTag::SER_ROUTER, "SendRequest error %{public}d", error);
        return static_cast<int32_t>(AbilityRuntime::AbilityErrorCode::ERROR_CODE_INNER);
