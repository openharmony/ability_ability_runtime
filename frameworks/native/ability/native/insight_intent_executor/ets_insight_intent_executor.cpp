...
{
    return new (std::nothrow) EtsInsightIntentExecutor(static_cast<ETSRuntime &>(runtime));
}

EtsInsightIntentExecutor::EtsInsightIntentExecutor(ETSRuntime &runtime) : runtime_(runtime)
{ }
...
bool EtsInsightIntentExecutor::Init(const InsightIntentExecutorInfo &insightIntentInfo)
{
    ...
    if ((status = env->FindClass(EXECUTOR_CLASS_NAME, &executorClass)) != ANI_OK) {
...
bool EtsInsightIntentExecutor::HandleExecuteIntent(
    ...
    bool &isAsync)
{
    ...
    switch (mode) {
        case InsightIntentExecuteMode::UIABILITY_FOREGROUND:
            if (!EtsInsightIntentExecutor::CheckParametersUIAbilityForeground(reinterpret_cast<ani_ref>(pageLoader))) {
                TAG_LOGE(AAFwkTag::INTENT, "CheckParametersUIAbilityForeground error");
                return ExecuteIntentCheckError();
            }
            ret = ExecuteInsightIntentUIAbilityForeground(name, *param, reinterpret_cast<ani_ref>(pageLoader));
            break;
        case InsightIntentExecuteMode::UIABILITY_BACKGROUND:
            ...
            ret = ExecuteInsightIntentUIAbilityBackground(name, *param);
            break;
        case InsightIntentExecuteMode::UIEXTENSION_ABILITY:
            if (!EtsInsightIntentExecutor::CheckParametersUIExtension(reinterpret_cast<ani_ref>(pageLoader))) {
                TAG_LOGE(AAFwkTag::INTENT, "CheckParametersUIExtension error");
                return ExecuteIntentCheckError();
            }
            ret = ExecuteInsightIntentUIExtension(name, *param, reinterpret_cast<ani_ref>(pageLoader));
            break;
        case InsightIntentExecuteMode::SERVICE_EXTENSION_ABILITY:
            ...
            ret = ExecuteInsightIntentServiceExtension(name, *param);
            break;
        default:
...
std::unique_ptr<AppExecFwk::ETSNativeReference> EtsInsightIntentExecutor::LoadEtsCode(
...
bool EtsInsightIntentExecutor::CheckParametersUIAbilityForeground(ani_ref windowStage)
{
...
bool EtsInsightIntentExecutor::CheckParametersUIExtension(ani_ref UIExtensionContentSession)
{
    ...
    ani_ref UIExtensionContentSession)
{
    ...
    ani_ref result = CallObjectMethod(true, "callOnExecuteInUIExtensionAbility", nullptr, aniName,
        aniWantParams, UIExtensionContentSession);
    if (result == nullptr) {
        return false;
    }
...
void EtsInsightIntentExecutor::OnExecuteResult(ani_env *env, ani_object aniObj, ani_object result)
{
    if (result == nullptr) {
...
ETS_EXPORT extern "C" OHOS::AbilityRuntime::InsightIntentExecutor *OHOS_ETS_Insight_Intent_Executor_Create(
    OHOS::AbilityRuntime::Runtime &runtime)
{
    return OHOS::AbilityRuntime::EtsInsightIntentExecutor::Create(runtime);
}
