...
void AppfreezeInner::SendProcessKillEvent(const std::string& killReason)
{
    ...
    if (applicationInfo != nullptr) {
        ...
        TAG_LOGI(AAFwkTag::APPDFR, "hisysevent write result=%{public}d, send event [FRAMEWORK,PROCESS_KILL], pid=%{public}d,"
            " processName=%{public}s, msg=%{public}s", result, pid, processName.c_str(), killReason.c_str());
    }
}
