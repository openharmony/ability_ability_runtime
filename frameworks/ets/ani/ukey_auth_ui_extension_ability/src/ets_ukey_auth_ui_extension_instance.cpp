...
namespace {
constexpr const char* ETS_ANI_LIBNAME = "libukey_auth_ui_extension_ani.z.so";
constexpr const char* ETS_ANI_CREATE_FUNC = "OHOS_ETS_UkeyAuth_Extension_Create";
using CreateETSUkeyAuthUIExtensionFunc = UkeyAuthUIExtension*(*)(const std::unique_ptr<Runtime>&);
CreateETSUkeyAuthUIExtensionFunc g_etsCreateFunc = nullptr;
}

UkeyAuthUIExtension *CreateETSUkeyAuthUIExtension(const std::unique_ptr<Runtime> &runtime)
{
    ...
    g_etsCreateFunc = reinterpret_cast<CreateETSUkeyAuthUIExtensionFunc>(symbol);
    return g_etsCreateFunc(runtime);
}
} // namespace AbilityRuntime
} // namespace OHOS
