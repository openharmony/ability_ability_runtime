...
#ifndef OHOS_ABILITY_RUNTIME_ETS_UI_EXTENSION_BASE_H
#define OHOS_ABILITY_RUNTIME_ETS_UI_EXTENSION_BASE_H
...
class EtsRuntime;
/**
 * @brief Ets ui extension base.
 */
class EtsUIExtensionBase : public UIExtensionBaseImpl, public std::enable_shared_from_this<EtsUIExtensionBase> {
public:
    explicit EtsUIExtensionBase(const std::unique_ptr<Runtime> &runtime);
    virtual ~EtsUIExtensionBase();
     ...
     * @return Ets extension common object.
     */
...
protected:
    ...
    ani_object CallObjectMethod(const char *name, const char *signature, ...);
    ...
    bool CallEtsOnSessionCreate(const AAFwk::Want &want, const sptr<AAFwk::SessionInfo> &sessionInfo,
...
protected:
    ...
    std::shared_ptr<AppExecFwk::ETSNativeReference> etsObj_;
...
#ifdef SUPPORT_GRAPHICS
private:
    class EtsUIExtensionBaseDisplayListener : public OHOS::Rosen::IDisplayInfoChangedListener {
    public:
        explicit EtsUIExtensionBaseDisplayListener(const std::weak_ptr<EtsUIExtensionBase> &EtsUiExtensionBase)
        {
            EtsUiExtensionBase_ = EtsUiExtensionBase;
        }
        ...
        {
            auto sptr = EtsUiExtensionBase_.lock();
            if (sptr != nullptr) {
    ...
    private:
        std::weak_ptr<EtsUIExtensionBase> EtsUiExtensionBase_;
    };
    ...
    sptr<EtsUIExtensionBaseDisplayListener> EtsUIExtensionBaseDisplayListener_ = nullptr;
#endif
};
} // namespace AbilityRuntime
} // namespace OHOS
#endif // OHOS_ABILITY_RUNTIME_ETS_UI_EXTENSION_BASE_H
