...
constexpr const char* GET_OBJECT_VOID_SIGNATURE = "i:Y";
...
bool GetFieldDoubleByName(ani_env *env, ani_object object, const char *name, double &value)
{
    ...
    if (isUndefined) {
        TAG_LOGE(AAFwkTag::BRIDGE, "%{public}s: undefined", name);
        return false;
    }
...
bool SetFieldDoubleByName(ani_env *env, ani_class cls, ani_object object, const char *name, double value)
{
    ...
    if (obj == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "CreateDouble failed");
        return false;
    }
...
bool SetFieldIntByName(ani_env *env, ani_class cls, ani_object object, const char *name, int32_t value)
{
    ...
    if (obj == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "CreateInt failed");
        return false;
    }
    if ((status = env->Object_SetField_Ref(object, field, reinterpret_cast<ani_ref>(obj))) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d", status);
...
bool SetFieldLongByName(ani_env *env, ani_class cls, ani_object object, const char *name, int64_t value)
{
    ...
    if (obj == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "CreateLong failed");
        return false;
    }
    if ((status = env->Object_SetField_Ref(object, field, reinterpret_cast<ani_ref>(obj))) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d", status);
...
bool GetFieldStringArrayByName(ani_env *env, ani_object object, const char *name, std::vector<std::string> &value)
{
    ...
    for (int i = 0; i < static_cast<int>(length); i++) {
        ...
        status = env->Object_CallMethodByName_Ref(reinterpret_cast<ani_object>(arrayObj),
            "$_get", GET_OBJECT_VOID_SIGNATURE, &stringEntryRef, (ani_int)i);
        if (status != ANI_OK) {
            TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d, index: %{public}d", status, i);
            return false;
        }
        std::string str = "";
        if (!GetStdString(env, reinterpret_cast<ani_string>(stringEntryRef), str)) {
            TAG_LOGE(AAFwkTag::BRIDGE, "GetStdString failed, index: %{public}d", i);
            return false;
        }
        value.emplace_back(str);
        TAG_LOGD(AAFwkTag::BRIDGE, "GetStdString index: %{public}d %{public}s", i, str.c_str());
    }
    return true;
}
    ...
    const std::vector<std::string> &value)
{
    ...
    for (size_t i = 0; i < value.size(); i++) {
        ...
        if ((status = env->Object_CallMethodByName_Void(arrayObj, "$_set", SET_OBJECT_VOID_SIGNATURE,
            i, str)) != ANI_OK) {
            TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d", status);
            return false;
        }
...
bool GetFieldRefByName(ani_env *env, ani_object object, const char *name, ani_ref &ref)
{
    ...
    if (isUndefined) {
        TAG_LOGE(AAFwkTag::BRIDGE, "%{public}s is undefined", name);
        return false;
    }
    return true;
}
...
ani_object CreateLong(ani_env *env, ani_long value)
{
    ...
    if (status != ANI_OK || cls == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "FindClass status: %{public}d, or null class", status);
        return nullptr;
    }
...
bool AsyncCallback(ani_env *env, ani_object call, ani_object error, ani_object result)
{
    ...
    if (status!= ANI_OK || clsCall == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "FindClass status: %{public}d, or null clsCall", status);
        return false;
    }
    ani_method method = nullptr;
...
bool GetDoubleOrUndefined(ani_env *env, ani_object param, const char *name, ani_double &value)
{
    ...
    if  (env == nullptr || param == nullptr || name == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null env or param or name");
        return false;
    }
...
bool GetPropertyRef(ani_env *env, ani_object obj, const char *name, ani_ref &ref, ani_boolean &isUndefined)
{
    ...
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Failed to get property '%{public}s', status: %{public}d", name, status);
        return false;
    }
    status = env->Reference_IsUndefined(ref, &isUndefined);
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Failed to check undefined for '%{public}s', status: %{public}d", name, status);
        return false;
    }
    return true;
}
...
bool SetOptionalFieldInt(ani_env *env, ani_class cls, ani_object object, const std::string &fieldName, int value)
{
    ...
    if (status != ANI_OK || field == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Class_FindField failed or null field, status=%{public}d, fieldName=%{public}s",
            status, fieldName.c_str());
        return false;
    }
    ani_object intObj = CreateInt(env, value);
    if (intObj == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null intObj");
        return false;
    }
    status = env->Object_SetField_Ref(object, field, intObj);
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Object_SetField_Ref failed, status=%{public}d, fieldName=%{public}s",
            status, fieldName.c_str());
        return false;
    }
    return true;
...
{
    if (env == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "env null or arrayObj null");
        return false;
    }
    ani_class arrayCls = nullptr;
    ...
    for (size_t i = 0; i < values.size(); i++) {
        ...
        if (aniStr == nullptr) {
            TAG_LOGE(AAFwkTag::BRIDGE, "null aniStr");
            return false;
        }
        if (status != ANI_OK) {
            TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d", status);
...
bool UnwrapArrayString(ani_env *env, const ani_object &arrayObj, std::vector<std::string> &stringList)
{
    ...
    for (idx = 0; idx < size; idx++) {
        if ((status = env->Array_Get(reinterpret_cast<ani_array>(arrayObj), idx, &ref)) != ANI_OK) {
            TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d, index: %{public}zu", status, idx);
            return false;
        }
        if (ref == nullptr) {
            TAG_LOGE(AAFwkTag::BRIDGE, "null ref");
            return false;
        }
        std::string str = "";
...
bool SetProcessInformation(ani_env *env, ani_object object, const AppExecFwk::RunningProcessInfo &processInfo)
{
    ...
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "bundleNames failed status:%{public}d", status);
        return false;
    }
    ...
    if ((status = env->Object_SetPropertyByName_Ref(object, "state", stateItem)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "state failed status:%{public}d", status);
        return false;
    }
    ...
    if ((status = env->Object_SetPropertyByName_Ref(object, "bundleType", bundleTypeItem)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "bundleType failed status:%{public}d", status);
        return false;
    }
    if (processInfo.appCloneIndex != -1 &&
        ...
        TAG_LOGE(AAFwkTag::BRIDGE, "appCloneIndex failed status:%{public}d", status);
        return false;
    }
    return true;
}
...
ani_object WrapProcessInformation(ani_env *env, const AppExecFwk::RunningProcessInfo &processInfo)
{
    ...
    if (cls == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "null cls");
        return nullptr;
    }
    if ((status = env->Class_FindMethod(cls, "<ctor>", ":", &method)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "find ctor failed status: %{public}d", status);
        return nullptr;
    }
    if ((status = env->Object_New(cls, method, &object)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Object_New ProcessInformationInner failed status: %{public}d", status);
        return nullptr;
    }
    if (object == nullptr) {
    ...
    if (!SetProcessInformation(env, object, processInfo)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "SetProcessInformation failed");
        return nullptr;
    }
    return object;
}
...
ani_object CreateRunningProcessInfoArray(ani_env *env, std::vector<AppExecFwk::RunningProcessInfo> infos)
{
    ...
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Object_New array status: %{public}d", status);
        return arrayObj;
    }
    ani_size index = 0;
    for (auto &processInfo : infos) {
        ...
        if (aniInfo == nullptr) {
            TAG_LOGW(AAFwkTag::BRIDGE, "null aniInfo");
            break;
        }
        status = env->Object_CallMethodByName_Void(arrayObj, "$_set", SET_OBJECT_VOID_SIGNATURE, index, aniInfo);
        if (status != ANI_OK) {
            TAG_LOGW(AAFwkTag::BRIDGE, "Object_CallMethodByName_Void failed status: %{public}d", status);
            break;
        }
        index++;
    }
...
ani_object CreateEmptyArray(ani_env *env)
{
    ...
    if (status != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Object_New array failed status: %{public}d", status);
        return nullptr;
    }
    return arrayObj;
}
...
bool GetStringArrayProperty(ani_env *env, ani_object param, const char *name, std::vector<std::string> &value)
{
    ...
    for (int i = 0; i < static_cast<int>(length); i++) {
        ...
        value.push_back(str);
        TAG_LOGI(AAFwkTag::BRIDGE, "GetStdString index: %{public}d %{public}s", i, str.c_str());
    }
...
bool GetDoublePropertyObject(ani_env *env, ani_object param, const char *name, double &value)
{
    ...
    if (!GetRefProperty(env, param, name, obj)) {
        TAG_LOGW(AAFwkTag::BRIDGE, "%{public}s : undefined", name);
        return false;
    }
    if ((status = env->Object_CallMethodByName_Double(
        reinterpret_cast<ani_object>(obj), "doubleValue", nullptr, &value)) != ANI_OK) {
...
bool SetStringArrayProperty(ani_env *env, ani_object param, const char *name, const std::vector<std::string> &values)
{
    ...
    for (size_t i = 0; i < values.size(); i++) {
        ...
        status = env->Object_CallMethodByName_Void(arrayObj, "$_set", SET_OBJECT_VOID_SIGNATURE, i, string);
        if (status != ANI_OK) {
            TAG_LOGE(AAFwkTag::BRIDGE, "status: %{public}d", status);
            return false;
        }
...
bool GetStaticFieldString(ani_env *env, ani_class classObj, const char *fieldName, std::string &value)
{
    ...
    if ((status = env->Class_FindStaticField(classObj, fieldName, &field)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Class_FindStaticField status: %{public}d", status);
        return false;
    }

    ani_ref obj = nullptr;
    if ((status = env->Class_GetStaticField_Ref(classObj, field, &obj)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Class_GetStaticField_Ref status: %{public}d", status);
        return false;
    }
    ...
    if (isUndefined) {
        TAG_LOGE(AAFwkTag::BRIDGE, "%{public}s : undefined", fieldName);
        return false;
    }
...
bool CheckCallerIsSystemApp()
{
    ...
    if (!Security::AccessToken::TokenIdKit::IsSystemAppByFullTokenID(selfToken)) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Non-system app forbidden to call");
        return false;
    }
    return true;
}
...
{
    TAG_LOGD(AAFwkTag::BRIDGE, "WrapLocale called");
    if (env == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "env is nullptr");
        return nullptr;
    }
    if (locale.empty()) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Locale string is empty");
        return nullptr;
    }
    ani_class localClass = nullptr;
    ani_status status = ANI_ERROR;
    if ((status = env->FindClass("std.core.Intl.Locale", &localClass)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Find class failed, status: %{public}d", status);
        return nullptr;
    }
    ...
    if ((status = env->Class_FindMethod(localClass, "<ctor>",
        ...
        TAG_LOGE(AAFwkTag::BRIDGE, "Find Locale constructor failed, status: %{public}d", status);
        return nullptr;
    }
...
ani_object CreateIntAniArray(ani_env *env, const std::vector<int32_t> &dataArry)
{
    ...
    if (status != ANI_OK || arrayCtor == nullptr) {
        TAG_LOGE(AAFwkTag::BRIDGE, "Class_FindMethod failed, status : %{public}d", status);
        return nullptr;
    }
    ...
    for (size_t i = 0; i < dataArry.size(); i++) {
        ...
        if (intObj == nullptr) {
            TAG_LOGE(AAFwkTag::BRIDGE, "intObj nullptr");
            return nullptr;
        }
        ani_status status =
            env->Object_CallMethodByName_Void(arrayObj, "$_set", SET_OBJECT_VOID_SIGNATURE, i, intObj);
        if (status != ANI_OK) {
            TAG_LOGE(
                AAFwkTag::BRIDGE, "Object_CallMethodByName_Void failed, status : %{public}d", status);
            return nullptr;
        }
    }
    return arrayObj;
