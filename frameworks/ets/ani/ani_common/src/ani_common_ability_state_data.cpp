    ...
    const AbilityStateData &data)
{
    ...
    if (!SetFieldStringByName(env, cls, object, "moduleName", data.moduleName)) {
        TAG_LOGE(AAFwkTag::ABILITYMGR, "set moduleName failed");
        return nullptr;
    }

    if (!SetFieldStringByName(env, cls, object, "bundleName", data.bundleName)) {
        TAG_LOGE(AAFwkTag::ABILITYMGR, "set bundleName failed");
        return nullptr;
    }

    if (!SetFieldStringByName(env, cls, object, "abilityName", data.abilityName)) {
    ...
    if (!SetFieldBoolByName(env, cls, object, "isAtomicService", data.isAtomicService)) {
...
{
    TAG_LOGD(AAFwkTag::ABILITYMGR, "call CreateAniAbilityStateDataArray, list.size=%{public}zu", list.size());
