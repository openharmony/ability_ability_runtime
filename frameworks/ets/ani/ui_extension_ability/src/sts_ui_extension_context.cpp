...
{
    TAG_LOGD(AAFwkTag::UI_EXT, "terminateSelfSync start");
...
{
    TAG_LOGD(AAFwkTag::UI_EXT, "TerminateSelfWithResultSync call");
    ani_class cls = nullptr;
    ani_long nativeContextLong;
    ani_field contextField = nullptr;
    ani_status status = ANI_ERROR;
    ...
    if ((status = env->Object_GetField_Long(obj, contextField, &nativeContextLong)) != ANI_OK) {
        TAG_LOGE(AAFwkTag::UI_EXT, "get filed status : %{public}d", status);
        return;
    }
