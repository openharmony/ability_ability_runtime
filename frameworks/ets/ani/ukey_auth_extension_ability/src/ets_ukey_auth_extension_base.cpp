...
void EtsUkeyAuthExtensionBase::RegisterUkeyContextConfigUpdateCallback()
{
    if (ukeyContext_ == nullptr || abilityInfo_ == nullptr) {
        TAG_LOGE(AAFwkTag::UI_EXT, "null ukeyContext_ or abilityInfo_");
        return;
    }
    auto ukeyExtensionAbility = std::static_pointer_cast<EtsUkeyAuthExtensionBase>(shared_from_this());
    std::weak_ptr<EtsUkeyAuthExtensionBase> abilityWptr = ukeyExtensionAbility;
    ...
    ukeyContext_->RegisterAbilityConfigUpdateCallback(
        ...
        std::shared_ptr<EtsUkeyAuthExtensionBase> abilitySptr = abilityWptr.lock();
