...
namespace {
const char *ETS_ANI_LIBNAME = "libshare_extension_ani.z.so";
const char *ETS_ANI_CREATE_FUNC = "OHOS_ETS_SHARE_Extension_Create";
using CreateETSShareExtensionFunc = ShareExtension*(*)(const std::unique_ptr<Runtime>&);
CreateETSShareExtensionFunc g_etsCreateFunc = nullptr;
}
...
ShareExtension *CreateETSShareExtension(const std::unique_ptr<Runtime> &runtime)
{
    ...
    g_etsCreateFunc = reinterpret_cast<CreateETSShareExtensionFunc>(symbol);
    return g_etsCreateFunc(runtime);
}
} // namespace AbilityRuntime
} // namespace OHOS